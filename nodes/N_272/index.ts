...
const NODE_ID = "NODE_272";
export default function NODE_272(convo: BotkitConversation): string {
  bkStrAsk(
    convo,
    `${NODE_ID}.hello`,
    async (answer, convo, bot, message) => {
      if (moment(answer).isValid()) {
        ...
        if (NoOfYears > 0) {
          convo.setVar("NO_OF_ZAKATSUPPLY_YEARS_LEFT", NoOfYears);
          convo.gotoThread("t_NODE_273");
        } else {
