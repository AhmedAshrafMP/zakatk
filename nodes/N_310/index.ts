...
const NODE_ID = "NODE_310";
export default function NODE_310(convo: BotkitConversation): string {
  bkStrAsk(
    convo,
    `${NODE_ID}.hello`,
    async (answer, convo, bot, message) => {
      if (moment(answer).isValid()) {
        ...
        if (NoOfYears > 0) {
          convo.setVar("NO_OF_ZAKATSTRUCTURE_LEFT", NoOfYears);
