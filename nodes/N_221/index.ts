...
const NODE_ID = "NODE_221";
export default function NODE_221(convo: BotkitConversation): string {
  bkStrAsk(
    convo,
    `${NODE_ID}.hello`,
    async (answer, convo, bot, message) => {
      if (moment(answer).isValid()) {
        ...
        if (NoOfYears > 0) {
          convo.setVar("NO_OF_RENTNOYEARS_LEFT", NoOfYears);
          convo.gotoThread("t_NODE_225");
        } else {
